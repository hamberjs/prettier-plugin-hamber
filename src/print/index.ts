...
import {
    checkWhitespaceAtEndOfHamberBlock,
    checkWhitespaceAtStartOfHamberBlock,
...
let hamberOptionsDoc: Doc | undefined;
...
export function print(path: FastPath, options: ParserOptions, print: PrintFn): Doc {
    ...
    const [open, close] = options.hamberStrictMode ? ['"{', '}"'] : ['{', '}'];
    const printJsExpression = () => [
        open,
        printJS(path, print, options.hamberStrictMode, false, false, 'expression'),
        close,
    ];
    ...
    switch (node.type) {
        ...
        case 'Options':
            if (options.hamberSortOrder !== 'none') {
        ...
        case 'Attribute': {
            if (isOrCanBeConvertedToShorthand(node)) {
                if (options.hamberStrictMode) {
                    return concat([node.name, '="{', node.name, '}"']);
                } else if (options.hamberAllowShorthand) {
                ...
                const quotes = !isLoneMustacheTag(node.value) || options.hamberStrictMode;
        ...
        case 'IfBlock': {
            const def: Doc[] = [
                '{#if ',
                printHamberBlockJS(path, print, 'expression'),
                '}',
                printHamberBlockChildren(path, print, options),
            ];
        ...
        case 'ElseBlock': {
            ...
            if (
                ...
                const def: Doc[] = [
                    '{:else if ',
                    path.map(
                        (ifPath) => printHamberBlockJS(ifPath, print, 'expression'),
                        'children',
                    )[0],
                    '}',
                    path.map(
                        (ifPath) => printHamberBlockChildren(ifPath, print, options),
                        'children',
                    )[0],
                ];
            ...
            return concat(['{:else}', printHamberBlockChildren(path, print, options)]);
        }
        case 'EachBlock': {
            const def: Doc[] = [
                '{#each ',
            ...
            if (node.key) {
                def.push(' (', printHamberBlockJS(path, print, 'key'), ')');
            }

            def.push('}', printHamberBlockChildren(path, print, options));
        ...
        case 'AwaitBlock': {
            ...
            if (!hasPendingBlock && hasThenBlock) {
                ...
                block.push(
                    groupConcat(['{#await ', printHamberBlockJS(path, print, 'expression'), '}']),
                );
        ...
        case 'PendingBlock':
        case 'CatchBlock':
            return printHamberBlockChildren(path, print, options);
        case 'EventHandler':
            return concat([
                'on:',
                node.name,
...
function printTopLevelParts(
    ...
    if (hamberOptionsDoc) {
        parts.options.push(hamberOptionsDoc);
    }

    const docs = flatten(parseSortOrder(options.hamberSortOrder).map((p) => parts[p]));

    // Need to reset these because they are global and could affect the next formatting run
    ignoreNext = false;
    ignoreRange = false;
    hamberOptionsDoc = undefined;
...
function printHamberBlockChildren(path: FastPath, print: PrintFn, options: ParserOptions): Doc {
    ...
    const whitespaceAtStartOfBlock = checkWhitespaceAtStartOfHamberBlock(node, options);
    const whitespaceAtEndOfBlock = checkWhitespaceAtEndOfHamberBlock(node, options);
    const startline =
...
function printChildren(path: FastPath, print: PrintFn, options: ParserOptions): Doc {
     ...
     * is done in parent already. By definition of the Hamber AST,
...
/**
 * `hamber:options` is part of the html part but needs to be snipped out and handled
...
function prepareChildren(
    ...
    let hamberOptionsComment: Doc | undefined;
    ...
    for (let idx = 0; idx < children.length; idx++) {
        ...
        if (options.hamberSortOrder !== 'none') {
            if (isCommentFollowedByOptions(currentChild, idx)) {
                hamberOptionsComment = printComment(currentChild);
            ...
            if (currentChild.type === 'Options') {
                printHamberOptions(currentChild, idx, path, print);
                continue;
            }
        }
    ...
    for (let idx = 0; idx < childrenWithoutOptions.length; idx++) {
        ...
        if (currentChild.type === 'Text' && nextChild && nextChild.type === 'Text') {
            // A tag was snipped out (f.e. hamber:options). Join text
    ...
    function printHamberOptions(
        node: OptionsNode,
        idx: number,
        path: FastPath,
        print: PrintFn,
    ): void {
        hamberOptionsDoc = groupConcat([
            groupConcat([
                '<',
                node.name,
                indent(
        ...
        if (hamberOptionsComment) {
            hamberOptionsDoc = groupConcat([hamberOptionsComment, hardline, hamberOptionsDoc]);
        }
    }
...
function printHamberBlockJS(path: FastPath, print: PrintFn, name: string) {
