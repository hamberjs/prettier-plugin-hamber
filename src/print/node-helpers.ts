...
/**
 * Check for a hamber block if there's whitespace at the start and if it's a space or a line.
 */
export function checkWhitespaceAtStartOfHamberBlock(
    node: Node,
    options: ParserOptions,
): 'none' | 'space' | 'line' {
    if (!isHamberBlock(node) || !isNodeWithChildren(node)) {
        return 'none';
    }
    ...
    // This extra check is necessary because the Hamber AST might swallow whitespace between
...
/**
 * Check for a hamber block if there's whitespace at the end and if it's a space or a line.
 */
export function checkWhitespaceAtEndOfHamberBlock(
    node: Node,
    options: ParserOptions,
): 'none' | 'space' | 'line' {
    if (!isHamberBlock(node) || !isNodeWithChildren(node)) {
...
export function isInsideQuotedAttribute(path: FastPath, options: ParserOptions): boolean {
    ...
    return stack.some(
        (node) =>
            node.type === 'Attribute' &&
            (!isLoneMustacheTag(node.value) || options.hamberStrictMode),
    );
}
