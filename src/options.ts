...
export interface PluginOptions {
    hamberSortOrder: SortOrder;
    hamberStrictMode: boolean;
    hamberBracketNewLine: boolean;
    hamberAllowShorthand: boolean;
    hamberIndentScriptAndStyle: boolean;
}
...
export const options: Record<keyof PluginOptions, SupportOption> = {
    hamberSortOrder: {
        ...
        choices: [
            ...
            makeChoice('none'),
            // Deprecated, keep in 2.x for backwards-compatibility. hamber:options will be moved to the top
    ...
    hamberIndentScriptAndStyle: {
        ...
        default: true,
        description:
            'Whether or not to indent the code inside <script> and <style> tags in Hamber files',
    },
};
...
export function parseSortOrder(sortOrder: SortOrder): SortOrderPart[] {
    ...
    if (!order.includes('options')) {
        console.warn(
            'hamberSortOrder is missing option `options`. This will be an error in prettier-plugin-hamber version 3.',
        );
...
export function isBracketSameLine(options: ParserOptions): boolean {
    return options.hamberBracketNewLine != null
        ? !options.hamberBracketNewLine
